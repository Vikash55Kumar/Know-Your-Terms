...
const agreementSummary = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
        ...
        const documentModelResponse = await axios.post(`${process.env.AI_DOCUMENT_MODEL_URL}/uploads`, formData, {
            headers: {
                ...formData.getHeaders(),
            },
        });
        ...
        formData1.append('document_text', agreementText);
    
        try {

            const summaryModelResponse = await axios.post(`${process.env.AI_SUMMARY_MODEL_URL}/summarize`, formData1, {
                headers: {
                    ...formData1.getHeaders(),
                },
            });
    
            if (!summaryModelResponse) {
                await createAuditLog({
                    uid,
                    action: 'AGREEMENT_SUMMARY',
                    status: 'failure',
            ...
            let summarizedDataResponse = summaryModelResponse.data;

            // console.log("Summary response extracted", summarizedDataResponse);

            // Translate the summary if needed
            // if (language && language !== 'en') {
            //     try {
            //         summarizedDataResponse = await translateText(summarizedDataResponse, language);
            ...
            return res.status(200).json(
                new ApiResponse(200, summarizedDataResponse, 'Agreement summarized successfully')
            );
